...
|---|---|---|
| `LLMWIKI_PROVIDER` | `trae` | Provider to use. One of: `anthropic`, `openai`, `ollama`, `minimax`, `copilot`, `claude-agent`, `codex-agent`, `trae`, `orcarouter`, `atlascloud` (aliases: `atlas-cloud`, `atlas`) |
| `LLMWIKI_MODEL` | Provider default | Model name to use. Overrides the provider's built-in default. Trae defaults to `Seed-Evolving`; Codex delegates to the CLI when unset. A set value is passed through with `--model` |
...
| `LLMWIKI_EMBEDDING_PROVIDER` | Active chat provider; `ollama` for Trae | Backend serving embeddings, independent of `LLMWIKI_PROVIDER`. One of `anthropic`, `claude-agent`, `openai`, `ollama`, `orcarouter` |
...
When an agent provider (`trae` or `codex-agent`) is selected, llmwiki does not
open the project `.env` file because that file may contain API keys. Export any
overrides in the shell for that run. Other providers retain the existing
shell-over-`.env` precedence.
...
| Variable | Required | Description |
|---|---|---|
| `LLMWIKI_PROVIDER` | No | Defaults to `trae`; set only to select a different provider |
| `LLMWIKI_MODEL` | No | Passed to `trae-cli exec --model`; defaults to `Seed-Evolving` |
| `LLMWIKI_EMBEDDING_PROVIDER` | No | Defaults to local `ollama`; override to use another embedding backend |
...
Like Codex, the CLI exposes no embeddings API and no `maxTokens` control. llmwiki
therefore uses local Ollama for Trae embeddings by default; its default embedding
model is `nomic-embed-text`. Each request has a fixed 10-minute timeout and 1 MiB
process/final-output caps.
