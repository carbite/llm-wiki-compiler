...
When you select the Anthropic provider, llmwiki resolves credentials in this order:
...
The Anthropic provider uses the official `@anthropic-ai/sdk` to call Claude directly. Select it with `LLMWIKI_PROVIDER=anthropic`.
...
# Optional overrides:
LLMWIKI_MODEL=another-model llmwiki compile
llmwiki compile --provider anthropic
```

Trae is the built-in default provider. llmwiki passes `Seed-Evolving` to
`trae-cli exec --model` and uses local Ollama with `nomic-embed-text` for
embeddings unless you override those settings.
...
<Warning>
  When `trae` is selected, including by default, llmwiki deliberately does not
  open the project `.env` file because it may contain API keys. Export any
  provider or model overrides in the shell for that run.
</Warning>
...
child in a throwaway directory, bounded timeout and output caps, the resolved
model passed to `--model`, no `maxTokens` control, and `--output-schema`-validated
structured output. Because TraeCode CLI has no embeddings interface, llmwiki
routes Trae embeddings to local Ollama by default. An explicit
`LLMWIKI_EMBEDDING_PROVIDER` still overrides that backend.
